...
public class AmazonPurchaseObserver extends BasePurchasingObserver {
    ...
    public void onSdkAvailable(final boolean isSandboxMode) {
        Log.v(TAG, "onSdkAvailable received: isSandboxMode=" + isSandboxMode);
    ...
    public void onGetUserIdResponse(final GetUserIdResponse response) {
        Log.v(TAG, "onGetUserIdResponse: response=" + response);
        new GetUserIdAsyncTask().execute(response);
    }
    ...
    public void onItemDataResponse(final ItemDataResponse response) {
        Log.v(TAG, "onItemDataResponse: response=" + response);
        new ItemDataAsyncTask().execute(response);
    }
    ...
    public void onPurchaseResponse(final PurchaseResponse response) {
        Log.v(TAG, "onPurchaseResponse: response=" + response);
        new PurchaseAsyncTask().execute(response);
    }
    ...
    public void onPurchaseUpdatesResponse(final PurchaseUpdatesResponse response) {
        Log.v(TAG, "onPurchaseUpdatesResponse: response=" + response);
        new PurchaseUpdatesAsyncTask().execute(response);
    }
    ...
    private SharedPreferences getSharedPreferencesForCurrentUser() {
        Log.v(TAG, "getSharedPreferencesForCurrentUser");
        return context.getSharedPreferences(getApplication().getCurrentUser(), Context.MODE_PRIVATE);
    }
    ...
    private class GetUserIdAsyncTask extends AsyncTask<GetUserIdResponse, Void, Boolean> {
        ...
        protected Boolean doInBackground(final GetUserIdResponse... params) {
            ...
            if (getUserIdResponse.getUserIdRequestStatus() == GetUserIdRequestStatus.SUCCESSFUL) {
                ...
                getApplication().setCurrentUser(userId);
                return true;
            } else {
        ...
        protected void onPostExecute(final Boolean result) {
            ...
            if (result) {
                PurchasingManager.initiatePurchaseUpdatesRequest(Offset.fromString(context
                        .getSharedPreferences(getApplication().getCurrentUser(), Context.MODE_PRIVATE)
                        .getString(OFFSET, Offset.BEGINNING.toString())));
            }
        }
    }

    private AmazonApplication getApplication() {
        return ((AmazonApplication) context.getApplicationContext());
    }
...
//                            editor.putInt(ButtonClickerActivity.NUM_CLICKS, numClicks + 10);
                        ...
                        case ENTITLED:
                            ...
                            break;
                        case SUBSCRIPTION:
                            final SubscriptionPeriod period = receipt.getSubscriptionPeriod();
...
//                    Log.v(TAG, "Invalid Sku for request " + baseActivity.requestIds.get(purchaseResponse.getRequestId()));
        ...
        protected void onPostExecute(final Boolean success) {
            ...
            if (success) {
                System.out.println("AmazonPurchaseObserver$PurchaseAsyncTask.onPostExecute");
...
//                        editor.putBoolean(ButtonClickerActivity.HAS_SUBSCRIPTION, hasSubscription);
        ...
        protected void onPostExecute(final Boolean success) {
            super.onPostExecute(success);
            if (success) {
                System.out.println("AmazonPurchaseObserver$PurchaseUpdatesAsyncTask.onPostExecute");
                context.sendBroadcast(new Intent(LibraryApplication.makeStateChangedBroadcast(context)));
            }
        }
    }
